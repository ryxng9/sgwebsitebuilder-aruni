...
const BUSINESS_QUERY = `*[_type == "work" && type == "business"] | order(publishedAt desc) {
  _id,
  title,
  slug,
  image,
...
async function getBusinessProjects(): Promise<WorkProject[]> {
  return client.fetch(BUSINESS_QUERY, {}, options);
}

export default async function BusinessWebDesignPage() {
  const businessProjects = await getBusinessProjects();
  return (
    <>
            ...
            {businessProjects.length === 0 ? (
              <div className="text-center py-20">
                <p className="font-sans text-white/60 text-lg">
                  No business projects yet. Add projects in the{" "}
                ...
                {businessProjects.map((project) => (
                  <div key={project._id} className="rounded-2xl border border-white/20 bg-white/5 backdrop-blur-sm p-8 lg:p-12 shadow-lg">
                    ...
                    <p className="font-sans text-white/85 text-base sm:text-lg leading-relaxed mb-6">
                      {project.description || "A business website showcasing our expertise."}
                    </p>
